...
async function fetchPosts() {
  try {
    ...
    for (const post of listOfPosts) {
      appendPost(post.title, post.body, post.id);
    }
  } catch (error) {
    alert(error.message);
  }
...
function appendPost(title, content, userId) {
  const postEl = document.importNode(postTemplate.content, true);
  postEl.querySelector('h2').textContent = title.toUpperCase();
  postEl.querySelector('p').textContent = content;
  postEl.querySelector('li').id = userId;
